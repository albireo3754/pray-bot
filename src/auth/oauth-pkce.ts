...
async function startCallbackServer(
  ...
  return new Promise<{ code: string }>((resolve, reject) => {
    ...
    try {
      server = Bun.serve({
        port,
        hostname: '127.0.0.1', // bind to loopback only — no external network access
        fetch(req) {
          ...
          if (state !== expectedState) {
            // Keep server alive — stray requests (browser extensions, redirects) should not
            // kill the login flow before the real OAuth callback arrives.
          ...
          if (!code) {
            // Keep server alive — malformed request, still waiting for real callback.
...
function parseTokenResponse(data: Record<string, unknown>, fallbackRefresh?: string): OAuthTokens {
  ...
  const refreshToken = (data.refresh_token as string | undefined) ?? fallbackRefresh;
  // Coerce explicitly: some OAuth servers return expires_in as a string (e.g. "3600").
  const expiresIn = Number(data.expires_in);
  const expiresInSafe = Number.isFinite(expiresIn) && expiresIn > 0 ? expiresIn : 3600;
  ...
  return {
    accessToken,
    refreshToken,
    expiresAt: Math.floor(Date.now() / 1000) + expiresInSafe,
