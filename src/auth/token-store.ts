...
export class TokenStore {
  ...
  private filePath(provider: string): string {
    return join(this.baseDir, `${this.validateProvider(provider)}.json`);
  }

  private lockPath(provider: string): string {
    return join(this.baseDir, `${this.validateProvider(provider)}.lock`);
  }

  private acquireLock(provider: string): void {
